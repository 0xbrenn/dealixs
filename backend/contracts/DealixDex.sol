...
contract DealixDEX is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // Add structs to handle complex function parameters
    ...
    function createTokenDiscountPool(
        ...
        require(params.discountPercentage > 0 && params.discountPercentage <= maxDiscountPercentage, "Invalid discount percentage");
    ...
    function _addLiquidity(
        ...
        IERC20(params.tokenA).transferFrom(msg.sender, address(this), totalA);
        IERC20(params.tokenB).transferFrom(msg.sender, address(this), totalB);
        
        // Approve router
        IERC20(params.tokenA).approve(address(router), params.amountADesired);
        IERC20(params.tokenB).approve(address(router), params.amountBDesired);
   ...
   function getDealixProfile(address user) external view returns (
    ...
    uint256 totalVolume,
    uint256 tier,
    uint256 badgeCount,  // Changed from 'badges' to 'badgeCount'
    uint256 socialPoints,
    uint256 swaps,
    uint256 streak,
    uint256 liquidityProvided,
    ...
    if (dealixID > 0) {
        ...
        badgeCount = profile.badgeCount;  // Changed here too
