...
async function main() {
  ...
  const DealixPair = await ethers.getContractFactory("DealixPair");
  ...
  if (match) {
    ...
    if (libraryHash !== currentInitCodeHash) {
      console.error("\n❌ ERROR: DealixLibrary.sol has not been updated with the correct init code hash!");
      console.error("Expected:", currentInitCodeHash);
      console.error("Found:", libraryHash);
      console.error("\nPlease update the hex value in DealixLibrary.sol to:", currentInitCodeHash.slice(2));
      process.exit(1);
    }
    console.log("✅ DealixLibrary.sol has correct init code hash");
  } else {
    console.error("❌ Could not find init code hash in DealixLibrary.sol");
    process.exit(1);
  }

  // Deploy the library
  console.log("\n📋 Deploying DealixLibrary");
  console.log("========================");
  
  try {
    const DealixLibrary = await ethers.getContractFactory("DealixLibrary");
