...
export default function App() {
  ...
  (window as any).findActualInitCodeHash = async () => {
  if (!walletProvider) return;
  
  const provider = new ethers.providers.Web3Provider(walletProvider);
  
  console.log("=== FINDING ACTUAL INIT CODE HASH ===");
  
  // Get the Pair contract bytecode to calculate its init code hash
  const pairArtifact = {
    bytecode: "" // This would be the actual bytecode
  };
  
  // Since we can't easily get the bytecode, let's reverse engineer it
  // We know the factory created the pair at a specific address
  // We can calculate what init code hash must have been used
  
  const FACTORY = "0xbF744FD5c3D4595E4f64A777ab67fDb130aBcbb0";
  const PAIR = "0xd5A0b4d67225a890aa96c1C786e4b5e14f21665e";
  const TEST_TOKEN = "0xa93f4e699E8a7B8C006D7e4ba44ecd2ba24954Bd";
  const WETH = "0x4200000000000000000000000000000000000006";
  
  // Get sorted tokens
  const token0 = TEST_TOKEN.toLowerCase() < WETH.toLowerCase() ? TEST_TOKEN : WETH;
  const token1 = TEST_TOKEN.toLowerCase() < WETH.toLowerCase() ? WETH : TEST_TOKEN;
  ...
  // The issue might be that your factory doesn't use CREATE2 at all
  // Let's check if it's using CREATE instead
  
  // First, let's check the factory's pair creation transaction
  console.log("\n🔍 Your factory might be using CREATE (not CREATE2) to deploy pairs!");
  console.log("This means the router CANNOT calculate pair addresses deterministically.");
  console.log("The router expects CREATE2, but your factory uses CREATE.");
  
  // Let's verify by checking if we can query the pair from factory
  const factory = new ethers.Contract(FACTORY, ["function getPair(address,address) view returns (address)"], provider);
  const pairFromFactory = await factory.getPair(token0, token1);
  
  console.log("\nFactory getPair result:", pairFromFactory);
  console.log("Matches deployed pair?", pairFromFactory.toLowerCase() === PAIR.toLowerCase() ? "✅ YES" : "❌ NO");
  
  console.log("\n❌ PROBLEM IDENTIFIED:");
  console.log("Your factory uses CREATE (sequential addresses) but the router expects CREATE2 (deterministic addresses)!");
  console.log("This is why the router can't find the pairs - it's calculating addresses that don't exist.");
};
...
const handleSwap = async () => {
  ...
  try {
    ...
    const expectedAmountOut = ethers.utils.parseUnits(toAmount, toToken.decimals);
    ...
    console.log('Swap details:', {
      ...
      minAmountOut: ethers.utils.formatUnits(minAmountOut, toToken.decimals),
    ...
    if (fromToken.address === '0x0000000000000000000000000000000000000000') {
      ...
      // Check if direct path exists, otherwise use WETH as intermediary
      const factoryContract = new ethers.Contract(FACTORY_ADDRESS, FACTORY_ABI, provider);
      const directPair = await factoryContract.getPair(fromToken.address, toToken.address);
      
      if (directPair === ethers.constants.AddressZero) {
        // No direct pair, use WETH as intermediary
        path.splice(1, 0, WETH_ADDRESS); // Insert WETH in the middle
        console.log('Using WETH intermediary path:', path);
      }
    ...
    if (error?.reason || error?.message) {
      ...
      if (message.includes('user rejected')) {
        ...
        errorMessage = 'Price moved, try increasing slippage tolerance';
