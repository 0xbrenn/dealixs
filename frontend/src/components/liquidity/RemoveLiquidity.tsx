...
interface RemoveLiquidityProps {
  ...
  receiveAsWETH?: boolean;
  onReceiveAsWETHChange?: (value: boolean) => void;
}
...
export const RemoveLiquidity: React.FC<RemoveLiquidityProps> = ({
  ...
  onSelectPosition,
  receiveAsWETH = false,
  onReceiveAsWETHChange
}) => {
  // Check if the selected position contains WETH
  const hasWETH = position && (
    position.token0.address.toLowerCase() === '0x4200000000000000000000000000000000000006'.toLowerCase() ||
    position.token1.address.toLowerCase() === '0x4200000000000000000000000000000000000006'.toLowerCase()
  );
      ...
      {position && (
        ...
        >
          ...
          <div className="space-y-4">
            ...
            {/* WETH Toggle Option - Only show if position contains WETH */}
            {hasWETH && onReceiveAsWETHChange && (
              <motion.div
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white font-medium">Receive as WETH</p>
                    <p className="text-gray-400 text-sm">Keep wrapped ETH instead of native ETH</p>
                  </div>
                  <button
                    onClick={() => onReceiveAsWETHChange(!receiveAsWETH)}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all duration-300 ${
                      receiveAsWETH 
                        ? 'bg-gradient-to-r from-[#00FF4D] to-cyan-500 shadow-[0_0_10px_rgba(0,255,77,0.4)]' 
                        : 'bg-gray-600'
                    }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-all duration-300 shadow-md ${
                        receiveAsWETH ? 'translate-x-6' : 'translate-x-1'
                      }`}
                    />
                  </button>
                </div>
            ...
            <div className="glass rounded-2xl p-4 space-y-3 border border-cyan-500/10">
              <div className="flex justify-between">
                <span className="text-gray-400">
                  ...
                  {hasWETH && position.token0.address.toLowerCase() === '0x4200000000000000000000000000000000000006'.toLowerCase() && !receiveAsWETH && (
                    <span className="text-xs text-cyan-400 ml-1">(as ETH)</span>
                  )}
                </span>
              ...
              <div className="flex justify-between">
                <span className="text-gray-400">
                  ...
                  {hasWETH && position.token1.address.toLowerCase() === '0x4200000000000000000000000000000000000006'.toLowerCase() && !receiveAsWETH && (
                    <span className="text-xs text-cyan-400 ml-1">(as ETH)</span>
                  )}
                </span>
                <span className="text-white font-medium">
