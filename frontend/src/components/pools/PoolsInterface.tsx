...
export const PoolsInterface: React.FC<PoolsInterfaceProps> = ({ walletProvider, onSelectPool }) => {
  ...
  const MobilePoolCard: React.FC<{ pool: Pool }> = ({ pool }) => (
    <motion.div
      whileTap={{ scale: 0.98 }}
      className="glass rounded-xl border border-white/10 p-4 hover:border-[#00FF4D]/30 transition-all duration-300"
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500 glass px-2 py-0.5 rounded border border-cyan-500/20">
                0.3%
              </span>
            </div>
          </div>
        </div>
        <span className="text-lg font-semibold bg-gradient-to-r from-[#00FF4D] to-cyan-500 bg-clip-text text-transparent">
      ...
      <div className="grid grid-cols-3 gap-2 text-sm">
        ...
        <div>
          ...
          <p className="text-[#00FF4D] font-medium">{formatCompactNumber(pool.fees24h)}</p>
        </div>
      </div>
    </motion.div>
  );
    ...
    <div className="w-full max-w-[1200px] mx-auto px-4 pb-20 pools-interface">
      ...
      >
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 pl-10 glass rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#00FF4D] border border-white/10 focus:border-[#00FF4D]/50 text-sm md:text-base transition-all duration-200"
            />
          ...
          <div className="flex items-center justify-between gap-3">
            <label className="flex items-center gap-2 cursor-pointer select-none">
              <input
                ...
                className="w-4 h-4 text-[#00FF4D] bg-gray-900 border-gray-600 rounded focus:ring-[#00FF4D] cursor-pointer accent-[#00FF4D]"
              />
            ...
            <button
              ...
              className="px-4 md:px-6 py-2 md:py-3 bg-gradient-to-r from-[#00FF4D] to-cyan-500 rounded-xl text-white font-medium hover:shadow-lg transition-all whitespace-nowrap text-sm md:text-base"
              style={{
        ...
        <div className="hidden lg:block glass rounded-2xl border border-white/10 overflow-hidden">
          {/* Table Header */}
          <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/10 text-sm text-gray-400 glass">
            ...
            {loading ? (
              <div className="px-6 py-20 text-center">
                <div className="inline-flex items-center gap-2 text-gray-400">
                  <svg className="animate-spin h-5 w-5 text-[#00FF4D]" viewBox="0 0 24 24">
              ...
              <div className="px-6 py-20 text-center">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="text-[#00FF4D] hover:text-[#00FF4D]/80 text-sm transition-colors"
                  >
                    Clear search
                  </button>
                )}
              ...
              filteredPools.map((pool) => (
                <motion.div
                  key={pool.address}
                  whileHover={{ backgroundColor: 'rgba(0, 255, 77, 0.03)' }}
                  className="grid grid-cols-12 gap-4 px-6 py-4 cursor-pointer transition-all hover:border-l-2 hover:border-l-[#00FF4D]"
                  onClick={() => handlePoolClick(pool)}
                >
                  {/* Pool Info */}
                  ...
                  <div className="col-span-2 text-right">
                    <p className="text-white font-medium bg-gradient-to-r from-[#00FF4D] to-cyan-500 bg-clip-text text-transparent">
                      {formatCompactNumber(pool.tvl)}
                    </p>
                  </div>
                  ...
                  {/* 24h Fees */}
                  <div className="col-span-2 text-right">
                    <p className="text-[#00FF4D]">{formatCompactNumber(pool.fees24h)}</p>
                  </div>
                </motion.div>
              ))
            )}
        ...
        {!loading && pools.length > 0 && (
          <div className="mt-6 flex items-center justify-between text-xs md:text-sm text-gray-400">
            ...
            <button
              ...
              className="flex items-center gap-2 hover:text-[#00FF4D] transition-colors px-3 py-1 rounded-lg hover:bg-[#00FF4D]/5"
            >
