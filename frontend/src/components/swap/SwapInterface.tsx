...
import { DiscountSelector } from '../dealix/DiscountSelector';
...
export const SwapInterface: React.FC<SwapInterfaceProps> = ({
  ...
  // Load discounts when tokens change
  useEffect(() => {
    if (hasDealixId && fromToken && toToken) {
      loadAvailableDiscounts(fromToken.address, toToken.address);
    }
  }, [fromToken, toToken, hasDealixId, loadAvailableDiscounts]);

  // Calculate enhanced output with discount
  const calculateEnhancedOutput = () => {
    if (!toAmount || totalDiscountPercentage === 0) return toAmount;
    const baseAmount = parseFloat(toAmount);
    const bonusAmount = baseAmount * (totalDiscountPercentage / 100);
    return (baseAmount + bonusAmount).toFixed(6);
  };

  const enhancedToAmount = calculateEnhancedOutput();
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <TokenInput
                value={hasDealixId && totalDiscountPercentage > 0 ? enhancedToAmount : toAmount}
                onChange={() => {}} // Read only
                token={toToken}
              ...
              {/* Discount Badge */}
              {hasDealixId && totalDiscountPercentage > 0 && toAmount && (
                <motion.div
                  initial={{ opacity: 0, scale: 0.8 }}
                  animate={{ opacity: 1, scale: 1 }}
                  className="absolute -top-2 -right-2 bg-gradient-to-r from-green-400 to-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg"
                >
            ...
            {/* Dealix Section - Show AFTER token inputs */}
            {isConnected && fromToken && toToken && (
              <>
                {/* Non-Dealix users prompt */}
                {!hasDealixId && (
                  <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="glass rounded-xl p-4 border border-green-500/30 bg-gradient-to-r from-green-500/5 to-blue-500/5"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <p className="text-green-400 font-semibold flex items-center space-x-2">
                          <span>💡</span>
                          <span>Create Dealix ID to unlock:</span>
                        </p>
                        <ul className="text-gray-300 text-sm mt-2 space-y-1">
                          <li className="flex items-center space-x-2">
                            <span className="text-green-400">•</span>
                            <span>Trading discounts (2-5%)</span>
                          </li>
                          <li className="flex items-center space-x-2">
                            <span className="text-green-400">•</span>
                            <span>Exclusive deals & rewards</span>
                          </li>
                          <li className="flex items-center space-x-2">
                            <span className="text-green-400">•</span>
                      ...
                      <button
                        onClick={() => setShowCreateDealixModal(true)}
                        className="ml-4 px-4 py-2 rounded-lg bg-gradient-to-r from-green-400 to-blue-500 text-white font-semibold hover:shadow-lg hover:shadow-green-400/30 transition-all whitespace-nowrap"
                      >
                        Create ID →
                      </button>
                    </div>
                ...
                {/* Dealix ID holders - Discount Selector */}
                {hasDealixId && fromAmount && parseFloat(fromAmount) > 0 && (
                  <DiscountSelector
                    tokenIn={fromToken.symbol}
                    tokenOut={toToken.symbol}
                    amountIn={fromAmount}
                    onDiscountChange={setTotalDiscountPercentage}
                  />
                )}
              </>
            )}
            ...
            {fromAmount && toAmount && parseFloat(fromAmount) > 0 && parseFloat(toAmount) > 0 && (
              ...
              >
                <div className="glass rounded-2xl p-4 space-y-3 border border-[#00FF4D]/20">
                  ...
                  {/* Show enhanced rate if discount applied */}
                  {hasDealixId && totalDiscountPercentage > 0 && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400 text-sm">Enhanced Rate</span>
                      <span className="text-green-400 text-sm font-medium">
                        1 {fromToken.symbol} = {(parseFloat(enhancedToAmount) / parseFloat(fromAmount)).toFixed(6)} {toToken.symbol}
                      </span>
                    </div>
                  )}
                  ...
                  {hasDealixId && totalDiscountPercentage > 0 && (
                    <div className="pt-2 border-t border-white/5">
                      <div className="flex justify-between items-center">
                        <span className="text-gray-400 text-sm">Your Savings</span>
                        <span className="text-green-400 text-sm font-bold">
                          +{((parseFloat(enhancedToAmount) - parseFloat(toAmount)) || 0).toFixed(6)} {toToken.symbol}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="pt-2 border-t border-white/5">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-gray-300 text-xs font-medium">
                        {(parseFloat(hasDealixId && totalDiscountPercentage > 0 ? enhancedToAmount : toAmount) * (1 - parseFloat(slippage) / 100)).toFixed(6)} {toToken.symbol}
                      </span>
                    </div>
                  </div>
                </div>
