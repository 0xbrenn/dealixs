...
export class SwapHandler {
  ...
  async executeSwap(
    ...
    const shouldUseDealix = this.dealixContract && hasDealixId && hasDiscounts;

    // Case 1: Native ETH to WETH (just wrap)
    if (this.isNativeToken(fromToken.address) && toToken.address === WETH_ADDRESS) {
    ...
    // Case 2: WETH to Native ETH (just unwrap)
    if (fromToken.address === WETH_ADDRESS && this.isNativeToken(toToken.address)) {
      const wethContract = new ethers.Contract(WETH_ADDRESS, WETH_ABI, this.signer);
    ...
    // For all other swaps, use Dealix if applicable
    if (shouldUseDealix) {
    ...
    } else if (this.dealixContract && !hasDealixId) {
      // Use Dealix's swapWithoutDealix for non-ID holders (applies platform fee)
      ...
      // Fallback to regular router swap if Dealix not available
