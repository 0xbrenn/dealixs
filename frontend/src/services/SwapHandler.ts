...
import { ROUTER_ADDRESS, ROUTER_ABI, ERC20_ABI, WETH_ADDRESS, WETH_ABI, DEALIX_ADDRESS, DEALIX_ABI } from '../constants/contracts';
...
export class SwapHandler {
 ...
 constructor(walletProvider: any) {
  ...
  if (DEALIX_ADDRESS && DEALIX_ADDRESS !== '') {
    console.log('Initializing Dealix contract at:', DEALIX_ADDRESS);
    this.dealixContract = new ethers.Contract(DEALIX_ADDRESS, DEALIX_ABI, this.signer);
  } else {
    console.warn('DEALIX_ADDRESS not found, Dealix features will be disabled');
...
async validateDiscountPool(
  poolId: number,
  tokenIn: string,
  tokenOut: string,
  amountIn: ethers.BigNumber
): Promise<{ isValid: boolean; reason?: string }> {
  if (!this.dealixContract) return { isValid: false, reason: 'No Dealix contract' };
  
  try {
    // Get discount pool details
    const pool = await this.dealixContract.discountPools(poolId);
    
    console.log('Discount Pool Details:', {
      poolId,
      creator: pool.creator,
      tokenA: pool.tokenA,
      tokenB: pool.tokenB,
      discountPercentage: pool.discountPercentage.toString(),
      minTradeSize: pool.minTradeSize.toString(),
      isActive: pool.isActive,
      expirationTime: new Date(pool.expirationTime.toNumber() * 1000).toISOString()
    });
    
    // Check if pool is active
    if (!pool.isActive) {
      return { isValid: false, reason: 'Discount pool is not active' };
    }
    
    // Check if pool is expired
    if (pool.expirationTime.toNumber() < Date.now() / 1000) {
      return { isValid: false, reason: 'Discount pool has expired' };
    }
    
    // Check if tokens match (considering WETH conversion)
    const poolTokens = [pool.tokenA.toLowerCase(), pool.tokenB.toLowerCase()];
    const swapTokens = [tokenIn.toLowerCase(), tokenOut.toLowerCase()];
    
    const tokensMatch = 
      (poolTokens.includes(swapTokens[0]) && poolTokens.includes(swapTokens[1])) ||
      (poolTokens.includes(swapTokens[1]) && poolTokens.includes(swapTokens[0]));
    
    if (!tokensMatch) {
      return { 
        isValid: false, 
        reason: `Token pair mismatch. Pool: ${pool.tokenA}/${pool.tokenB}, Swap: ${tokenIn}/${tokenOut}` 
      };
    }
    
    // Check minimum trade size
    if (amountIn.lt(pool.minTradeSize)) {
      return { 
        isValid: false, 
        reason: `Amount below minimum trade size: ${ethers.utils.formatEther(pool.minTradeSize)} required` 
      };
    }
    
    return { isValid: true };
  } catch (error) {
    console.error('Error validating discount pool:', error);
    return { isValid: false, reason: 'Failed to validate discount pool' };
  }
}
  ...
  if (!spender || spender === '') {
    throw new Error('Invalid spender address for token approval');
  }
...
private async executeSwapWithDealix(
  ...
  // Validate discount pools before proceeding
  for (const poolId of discountPoolIds) {
    const validation = await this.validateDiscountPool(poolId, tokenIn, tokenOut, amountIn);
    if (!validation.isValid) {
      throw new Error(`Discount pool ${poolId} validation failed: ${validation.reason}`);
    }
  }

  // CRITICAL FIX: For ETH swaps, we need to handle it differently
  if (this.isNativeToken(fromToken.address)) {
    // For ETH -> Token swaps through Dealix, we need to check if the contract
    // expects ETH to be sent as value or if it expects WETH approval
    
    // First, check if user has WETH and if not, wrap ETH first
    ...
    console.log('Required amount:', ethers.utils.formatEther(amountIn));
    
    if (wethBalance.lt(amountIn)) {
      console.log('Insufficient WETH, wrapping ETH first...');
      
      // Calculate how much ETH to wrap
      const amountToWrap = amountIn.sub(wethBalance);
      
      // Wrap ETH to WETH
      const wrapTx = await wethContract.deposit({ value: amountToWrap });
      console.log('Wrapping ETH to WETH...');
      await wrapTx.wait();
      console.log('ETH wrapped successfully');
    }
    
    // Now approve WETH for Dealix contract
    console.log('Approving WETH for Dealix contract...');
    await this.handleTokenApproval(WETH_ADDRESS, amountIn, DEALIX_ADDRESS, userAddress);
  } else {
    // For token swaps, handle approval normally
    console.log('Approving token for Dealix contract...');
  ...
  console.log('Executing Dealix swap with params:', {
    ...
    amountInFormatted: ethers.utils.formatUnits(amountIn, fromToken.decimals),
    minAmountOut: minAmountOut.toString(),
    minAmountOutFormatted: ethers.utils.formatUnits(minAmountOut, toToken.decimals),
    discountPoolIds,
  ...
  // IMPORTANT: Do NOT send ETH as value since we've already wrapped it to WETH
  const tx = await this.dealixContract.swapWithDealix(swapParams, { 
    gasLimit: 500000 
  });
